...
# Ruta de la nueva carpeta (sube un nivel desde 'server' y crea 'graficas_memoria')
...
ruta_acc = os.path.join(CARPETA_GRAFICAS, 'grafica_accuracy.png')
...
# Guardamos en la nueva carpeta
ruta_loss = os.path.join(CARPETA_GRAFICAS, 'grafica_loss.png')
plt.savefig(ruta_loss, dpi=300)
print(f"Gráfica guardada en: {ruta_loss}")

# ==========================================
# GRÁFICA 3: Tráfico de Red (KB)
# ==========================================
plt.figure(figsize=(8, 5))
sns.barplot(data=df, x='Ronda', y='Trafico_Ronda_KB', hue='Experimento_ID', alpha=0.8)

plt.title('Consumo de Ancho de Banda por Ronda', fontsize=14, fontweight='bold')
plt.xlabel('Ronda de Comunicación', fontsize=12)
plt.ylabel('Tráfico Transferido (KB)', fontsize=12)
plt.legend(title='Experimento', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()

# Guardamos en la nueva carpeta
ruta_trafico = os.path.join(CARPETA_GRAFICAS, 'grafica_trafico.png')
plt.savefig(ruta_trafico, dpi=300)
print(f"Gráfica guardada en: {ruta_trafico}")
